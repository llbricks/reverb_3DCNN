...
%
% Something was wrong before; now, just delay the RF real signal
% and get the IQ data afterwards. Do it by upsampling z initially
function [focsig_noise, focsig_noNoise, thermal_dB, reverb_dB] = add_noise_and_focus_file(simfile,apply_aberration)

%     addpath /data/dhyun/code/f2p_sims/functions/
...
%     disp(['Focusing...'])
    ...
    addpath /data/llbricks/3d/US_denoising/preprocessing/field2/matlab_helper_functions
    correlation_length = 4E-3;
    aberration = compute_aberration(par, correlation_length);  
...
%         disp(T)
        ...
        % Beamform both the noisy and noise free data, with ab if specified
        if apply_aberration
            delayRT_ab = delayRT + aberration(T) + repmat(aberration',size(delayRT,1),1);
            delayRT_ab_t = single((delayRT_ab + par.tshift) * par.fs - start_sample+1);
            delayRT_t = single((delayRT + par.tshift) * par.fs - start_sample+1);
            focsig_noise = focsig_noise + beamform_fullSynth_f2p(rfdata_noise,delayRT_ab_t,chanmat,nix,niy,niz,nelems,T);
            focsig_noNoise = focsig_noNoise + beamform_fullSynth_f2p(rfdata_noNoise,delayRT_ab_t,chanmat,nix,niy,niz,nelems,T);
        else 
            delayRT_t = single((delayRT + par.tshift) * par.fs - start_sample+1);
